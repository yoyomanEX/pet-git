...
jq2(document).ready(
	function() {
		...
		function() {
			jq2("#somedivS").empty();
			jq2.getJSON('queryUnprocessed.json',
				...
				'<table border="1">')
					.appendTo(
					jq2('#somedivS'));
					jq2.each(categoriesJson,
		             function(index, element) {
						('<tr>').appendTo($table)
						.append($('<td>').text(element.pk))
						.append($('<td>').text(element.id))
						.append($('<td>').text(element.name + "(JQuery版)"))
						.append($('<td>').text(element.balance))
						.append($('<td>').text(element.birthday));
					  });
				});
			});
			});
						...
						<div class="card-header py-3">
								<h6 class="m-0 font-weight-bold text-primary">
									<a> <label for="from">訂單起日</label>
										...
										<input type="hidden" value='${CompanyLoginOK.company_id}' name='company_id'>
										<label
										for="to">~訂單迄日</label> 
										...
										<button id='clickmeQ'>查詢(JQuery)</button>
									</a>
									<a href="orderManagement">返回訂單管理</a> 
									<input type="hidden"
										value='${CompanyLoginOK.company_id}' name='company_id'
										id="company_id">
								</h6>
						</div>
<!-- 						內文開始 -->
